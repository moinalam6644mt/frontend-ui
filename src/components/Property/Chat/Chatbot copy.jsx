...
const Chatbot = () => {
  ...
  const submitInputTypeValue = () => {
    ...
    const isQ22 = question[currentIndex].key === "Q22";

    const nextIndex =
      currentIndex < question.length - 1 ? currentIndex + 1 : currentIndex;

    const isSpecialQuestion = ["Q6", "Q7", "Q10", "Q11", "Q22"].includes(
      question[currentIndex].key
    );
    ...
    if (currentIndex < question.length - 1) {
      ...
      setUserData((prevUserData) => {
        ...
        if (!isQ6 && !isQ7 && !isQ10 && !isQ11 && !isQ22) {
                    ...
                    <div key={index} className="chat-box-wrapper">
                      ...
                      {val.type === "flat_processing_type" && (
                        <div className="col-lg-8 col-12">
                          ...
                          {inputValues &&
                            userData[index].key === "Q22" && (
                              <div
                                className="chat-box"
                                style={{ fontSize: "20px", color: "black" }}
                              >
                                <Availability setinputfeildvalue ={setinputfeildvalue}/>
                              </div>
                            )}
                        </div>
                      )}
